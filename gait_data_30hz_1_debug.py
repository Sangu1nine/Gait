"""
보행 및 낙상 감지 IMU 센서 데이터 수집 프로그램 (디버깅 버전)
MODIFIED 2025-01-30: 디버깅 로그 추가 - 프레임 처리 과정 상세 추적
Features:
...
# Debug variables
debug_frame_skips = 0
debug_prediction_count = 0
debug_sensor_frame_count = 0

# Fall detection variables
fall_interpreter = None
fall_scalers = {}  # Dictionary for multiple scallers
...
def sensor_collection_thread():
    ...
    global raw_sensor_buffer, is_running, debug_sensor_frame_count
    ...
    while is_running:
        try:
            ...
            frame_count += 1
            debug_sensor_frame_count = frame_count
...
def gait_detection_thread():
    ...
    global debug_frame_skips, debug_prediction_count
    ...
    while is_running:
        try:
            ...
            with sensor_data_lock:
                ...
                buffer_first_frame = raw_sensor_buffer[0]['frame']
                buffer_last_frame = raw_sensor_buffer[-1]['frame']
                buffer_size = len(raw_sensor_buffer)
            
            # Get current frame number
            current_frame = sensor_window[-1]['frame']
            window_first_frame = sensor_window[0]['frame']
            ...
            # Debug logging every 100 predictions
            if debug_prediction_count % 100 == 0:
                print(f"🔧 DEBUG - Buffer: {buffer_size}, Buffer range: [{buffer_first_frame}-{buffer_last_frame}], "
                      f"Window range: [{window_first_frame}-{current_frame}], "
                      f"Last processed: {last_prediction_frame}, Skips: {debug_frame_skips}")
...
def main():
    ...
    print("=" * 60)
    print("🚶 Gait & Fall Detection System (Debug Version)")
    print("=" * 60)
        ...
        while True:
            ...
            if int(time.time()) % 5 == 0:
                ...
                print(f"📊 Status - Buffer: {buffer_size}, State: {gait_state}, "
                      ...
                      f"Sensor frames: {debug_sensor_frame_count}, Predictions: {debug_prediction_count}")
